        ...
        <c:when test="${not empty emfMap.record}">
            <c:forEach items="${emfMap.record}" var="record" varStatus="status">
                <tr>
                    ...
                    <td>
                    	<!-- ${record.rsltNm} -->
                    	<!-- 반영 : http://183.111.69.197:8080/issues/72 -->
                    	<c:choose>
                    		<c:when test="${'F' eq record.rsltCd}">
		                    	<c:choose>
		                    		<c:when test="${not empty record.fileNo}">
		                    			<c:choose>
		                    				<c:when test="${record.rsltCd eq 'S' or record.rsltCd eq 'F'}">Y</c:when>
                    ...
                    <td>
                    	<c:choose>
                    		<c:when test="${'F' eq record.rsltCd}">
                    			${egov:convertDate(record.fileNo, 'yyyyMMdd', 'yyyy-MM-dd', '')}
                    		</c:when>
                    		<c:otherwise>
                    		</c:otherwise>
                   		</c:choose>
