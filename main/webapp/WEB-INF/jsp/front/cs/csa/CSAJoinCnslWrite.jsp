<%@ page pageEncoding="UTF-8" contentType="text/html; charset=UTF-8"%><%@include file="/WEB-INF/jsp/include/el.jspf" %>
						...
						<div class="termDiv">
							...
							<div class="termBox">
								<p class="tit">[대명라이프웨이 상품 가입을 위한 개인정보 수집, 이용 및 위탁동의서]<br /><br />주식회사 대명스테이션은 대명라이프웨이 상품과 관련하여 아래와 같이 개인정보를 수집, 이용 및 위탁하고자 합니다.</p>
								<br />
								<p class="tit">■ 개인정보 수집, 이용 내역</p>
								<p> 
									- 수집/이용자 : 주식회사 대명스테이션<br /> 
									- 수집/이용 개인정보 항목 : 성명, 연락처(휴대전화번호), 이메일<br /> 
									- 수집/이용 목적 : 대명라이프웨이 상품 소개, 계약상담 및 계약체결<br />
							...
							<div class="boardType2 mt15">
								<table summary="">
									...
									<tbody>
										<tr>	
											...
											<td>
												<select name="inqryCd" title="문의 구분" class="inputType1">
													...
													<c:forEach var="list" items="${rtnMap.joinInqryGb}" varStatus="status">
										...
										<tr>	
											<th scope="row"><span class="essen" title="필수입력">통화 가능 시간</span></th>
											<td>
												<div class="date">
													<input type="text" id="telAbleDt" name="telAbleDt" value="" title="날짜입력" class="inputType1 datepicker_input" style="" readonly="readonly" />
													<a href="javascript:" onclick="jQuery(this).prev().focus();"><img alt="날짜" src="/common/images/icon/calendar_icon.gif" /></a>
												</div>
												<select id="telAbleStrtTime" name="telAbleStrtTime" title="통화가능시간" class="inputType3">
												...
												~
												<select id="telAbleEndTime" name="telAbleEndTime" title="통화가능시간" class="inputType3">
													<c:forEach var="i" begin="0" end="23" step="1" varStatus="status">
														<c:choose>
															<c:when test="${i < 10}">
																<option value="0${i}">0${i}:00</option>
							...
							jQuery(document).ready(function(){
								...
								jQuery("#frm").validation({
							        ...
							        customfunc : function(obj, id) {
							        	...
							        	//통화가능시간 유효성
							        	if("telAbleStrtTime" == id || "telAbleEndTime" == id)
										{
											var telAbleTime = jQuery(obj).val();

											if(telAbleTime.length != 2 || parseInt(telAbleTime) > 23)
							        		{
							        			alert("통화가능시간 입력값이 올바르지 않습니다.");
							        			jQuery(obj).focus();
							        			return false;
							        		}
							        		
							        		if("telAbleEndTime" == id)
							        		{
							        			if(parseInt(telAbleTime) < parseInt(jQuery("#telAbleStrtTime").val()))
							        			{
							        				alert("통화가능 시작시간이 종료시간보다 클 수 없습니다.");
								        			jQuery(obj).focus();
								        			return false;
							        			}
							        		}
