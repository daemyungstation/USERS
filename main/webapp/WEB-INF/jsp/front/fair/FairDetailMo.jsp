...
/* 		.input .form td.write div.inputround { box-sizing: border-box; border: 1px solid #eeeeee; border-radius: 20px; background:#ffffff; height:40px; line-height:40px; } */
		...
		.input .form .input_rule .agree_text { position:absolute; top:30px; left:-2px; background-color:#ffffff; box-sizing:border-box; z-index:1; display:none; }
		...
		.input .form .input_rule .agree_text .agree_telegram { color:#717171; font-weight:400; font-size:19px; letter-spacing: -2px; text-align:left; line-height: 140%; word-break:break-all; }
		...
		#benefitList .benefitContainer .benefitList .benefitLi .benefitList_img img { max-width:150px; max-height:150px; min-height:90px; }
	...
	<script>
			...
			jQuery("textarea[name='INPUT']").each(function(){
				...
				for(var i = 0; i < jsonInput.INPUT.length; i++) {
					...
					//결합혜택
						...
						html += '<td class="write col3 txt color"><div class="inputround">';
						html += '<a onclick="javascript:jQuery(this).benefit_list();" id="benefit_product">선택하기</a></div></td>';
						html += '</tr>';
					}
			...
			jQuery("textarea[name='DEATIL']").each(function(){
				...
				for(var i = 0; i < jsonDetail.length; i++) {
					...
					if(i == jsonDetail.length -1) {
						html += '<div class="detail_content"><img src="/common/images/fairimage/'+ detail.M_SAVE_NM +'" alt="'+ detail.M_REAL_NM +'" onload="javascript:load_detail();"></div>';
					}else {
		...
		function onScroll() {
			var scrollTop = jQuery("#detail").offset().top;
...
// 						html += '</ul>';
	...
	<div class="topMove"><a style="cursor:pointer;" onclick="javascript:moveTop();"><img class="logo" src="/common/images/fairimage/btn_top_mo.png"></a></div>
